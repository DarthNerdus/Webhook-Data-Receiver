...
function get_quest_task(MAIN, quest) {
  ...
  switch (true) {
    ...
    case quest.template.indexOf('add_friend') >= 0:
      quest_task = 'Add ' + quest.target + ' Friends'; break;

    // SNAPSHOTS
    case quest.template.indexOf('snapshot') >= 0:
      quest_task = 'Take ' + quest.target + ' Snapshots.'; break;

    // ROCKET
    case quest.template.indexOf('rocket') >= 0:
      quest_task = 'Battle ' + quest.target + ' Team Rocket Grunts.'; break;

    // PURIFY BATTLE
    case quest.template.indexOf('purify') >= 0:
      quest_task = 'Purify ' + quest.target + ' Pokémon.'; break;
