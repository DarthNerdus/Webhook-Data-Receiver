  ...
  MAIN.pdb.query(`SELECT * FROM users WHERE user_id = ? AND discord_id = ?`, [message.member.id, message.guild.id], function (error, user, fields) {
    ...
    if (!user[0].raids) {
      ...
      if (!raid.subscriptions[0]) {
        ...
        // IF RAID SUBSCRIPTIONS OVER 25
        if (raid.subscriptions.length > 25) {
          for (let e = 0; e < raid.subscriptions.length; e++) {
            if (e == 24) {
              message.channel.send(raid_subs).catch(console.error);
              raid_subs.fields = [];
            }

            if (raid.subscriptions[e].gym != 'All' && raid.subscriptions[e].boss != 'All') {
              title = '#' + (e + 1) + ' ' + raid.subscriptions[e].boss;
              body = 'Gym: ' + raid.subscriptions[e].gym + '\nFiltered by Areas: `' + raid.subscriptions[e].areas + '`';
            } else if (raid.subscriptions[e].gym != 'All' && raid.subscriptions[e].boss == 'All' && raid.subscriptions[e].min_lvl == '1' && raid.subscriptions[e].ax_lvl == '5') {
              title = '#' + (e + 1) + ' ' + raid.subscriptions[e].gym;
              body = 'All Levels`\nFiltered by Areas: `' + raid.subscriptions[e].areas + '`';
            } else if (raid.subscriptions[e].gym != 'All' && raid.subscriptions[e].boss == 'All') {
              title = '#' + (e + 1) + ' ' + raid.subscriptions[e].gym;
              body = 'Min/Max Lvl: `' + raid.subscriptions[e].min_lvl + '/' + raid.subscriptions[e].max_lvl + '`\nFiltered by Areas: `' + raid.subscriptions[e].areas + '`';
            } else if (raid.subscriptions[e].gym == 'All' && raid.subscriptions[e].boss != 'All') {
              title = '#' + (e + 1) + ' ' + raid.subscriptions[e].boss;
              body = 'All Gyms\nFiltered by Areas: `' + raid.subscriptions[e].areas + '`';
            } else if (raid.subscriptions[e].gym == 'All' && raid.subscriptions[e].boss == 'All') {
              if (raid.subscriptions[e].min_lvl == raid.subscriptions[e].max_lvl) {
                title = '#' + (e + 1) + ' Level ' + raid.subscriptions[e].max_lvl + ' Raids';
              } else {
                title = '#' + (e + 1) + ' Level ' + raid.subscriptions[e].min_lvl + ' - ' + raid.subscriptions[e].max_lvl + ' Raids';
              } body = '**All Gyms**\nFiltered by Areas: `' + raid.subscriptions[e].areas + '`';
            } else {
              title = '#' + (e + 1);
              body = 'Gym: `' + raid.subscriptions[e].gym + '`\nRaid Boss: `' + raid.subscriptions[e].boss + '`\nMin/Max Lvl: `' + raid.subscriptions[e].min_lvl + '/' + raid.subscriptions[e].max_lvl + '`\nFiltered by Areas: `' + raid.subscriptions[e].areas + '`';
            }
